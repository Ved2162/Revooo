                  ...
                  {suggestions.map((suggestion, index) => (
                    <button
                      ...
                      className={`w-full text-left px-4 py-3 hover:bg-gray-50 focus:bg-gray-50 focus:outline-none border-b border-gray-100 last:border-b-0 first:rounded-t-lg last:rounded-b-lg ${
                        index === selectedSuggestionIndex ? 'bg-emerald-50 dark:bg-emerald-950/40 text-blue-900' : ''
                      }`}
                      onMouseDown={(e) => {
                        e.preventDefault();
                      }}
          ...
          <div className="flex-1">
            {hasSearched && (query || Object.keys(filters).length > 0) && (
              <div className="mb-6 flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                ...
                <select 
                  className="text-sm border border-gray-300 rounded-md px-3 py-2 w-full sm:w-48 focus:ring-2 focus:ring-emerald-500"
                  onChange={(e) => {
                    // Handle sorting
                  }}
                >
                  ...
                  return (
                    <Card key={facility.hashId} className="group overflow-hidden hover:shadow-lg transition-shadow">
                      <div className="relative h-48 overflow-hidden">
                        ...
                        {facility.distance && (
                          <Badge 
                            className="absolute top-3 right-3 bg-emerald-600 text-white text-xs"
                          >
                      ...
                      <CardFooter className="p-4 pt-0 flex flex-col sm:flex-row gap-2">
                        ...
                        <Button className="w-full text-sm" onClick={() => checkAuthForBooking(facility.id)}>
                            Book Now
                          </Button>
                      </CardFooter>
                    </Card>
