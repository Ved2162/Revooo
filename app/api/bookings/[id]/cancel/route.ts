...
export async function POST(
  ...
  if (!process.env.DATABASE_URL?.trim()) {
    ...
    if (booking.status !== 'CONFIRMED') return Response.json({ error: 'Only confirmed bookings can be cancelled' }, { status: 400 });
    return Response.json({ message: 'Booking cancelled successfully', booking: { id: booking.id, status: 'CANCELLED', cancelledAt: new Date().toISOString() } });
  }
