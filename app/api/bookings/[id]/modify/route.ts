...
export async function POST(
  ...
  if (!process.env.DATABASE_URL?.trim()) {
    ...
    if (!newDate || !newTime) return Response.json({ error: 'New date and time are required' }, { status: 400 });
    return Response.json({ message: 'Booking modified successfully', booking: { id: booking.id, status: booking.status, bookingDate: newDate, startTime: newTime, endTime: `${String(Number(newTime.slice(0, 2)) + 1).padStart(2, '0')}:00`, finalAmount: booking.finalAmount, modificationFee: 0 } });
  }
