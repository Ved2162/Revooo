  ...
  try {
    if (!process.env.DATABASE_URL?.trim()) {
      ...
      const filteredVenues = demoVenues.filter((venue) => (
        ...
        (!search || `${venue.name} ${venue.city} ${venue.description}`.toLowerCase().includes(search))
      ));
      const start = (page - 1) * limit;
      const venues = filteredVenues.slice(start, start + limit).map((venue) => ({
        id: venue.id,
        name: venue.name,
        ...
        maxPrice: Math.max(...venue.courts.map((court) => court.pricePerHour)),
        sportTypes: [...new Set(venue.courts.map((court) => court.sportType))],
      ...
      return NextResponse.json({
        ...
        pagination: {
          currentPage: page,
          totalPages: Math.max(1, Math.ceil(filteredVenues.length / limit)),
          totalCount: filteredVenues.length,
          hasNextPage: start + venues.length < filteredVenues.length,
          hasPreviousPage: page > 1,
          limit,
        },
        filters: {
          cities: [...new Set(demoVenues.map((venue) => venue.city))],
          states: [...new Set(demoVenues.map((venue) => venue.state))],
          priceRange: { min: 300, max: 1200 },
          sportTypes: [...new Set(demoVenues.flatMap((venue) => venue.courts.map((court) => court.sportType)))],
          venueTypes: [...new Set(demoVenues.map((venue) => venue.venueType))],
        },
      });
    }
