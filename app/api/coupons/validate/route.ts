...
export async function POST(request: NextRequest) {
  try {
    if (!process.env.DATABASE_URL?.trim()) {
      ...
      if (!code || !totalAmount) {
        return Response.json({ error: "Coupon code and total amount are required" }, { status: 400 });
      }
      const upperCode = code.toUpperCase();
      if (upperCode === "WELCOME10") {
        const discount = Math.min(totalAmount * 0.10, 500);
        return Response.json({
          valid: true,
          coupon: {
            id: "demo-coupon-welcome",
            code: "WELCOME10",
            name: "Welcome Offer",
            description: "10% off up to ₹500 on your first booking",
